...
class F_fully_connected(nn.Module):
    ...
    def __init__(self, size_in, size, channels_hidden=None, dropout=0.0):
        ...
        self.fc3 = nn.Linear(channels_hidden * 2, size)
        self.nl1 = nn.ReLU()
        self.nl2 = nn.ReLU()
        self.nl2b = nn.ReLU()

        self.attention = Class_Attention(dim=size_in, num_heads=4)
        self.attention = BRA(dim=size_in,topk=64)
        # layer scale
        self.gamma = nn.Parameter(torch.ones(channels_hidden), requires_grad=True)
        # self.gamma = nn.Parameter(torch.zeros(channels_hidden),requires_grad=True)
        # mean pooling
        # self.pool = nn.AdaptiveAvgPool2d((1, 1))
        # res
        # self.bn = nn.BatchNorm1d(size_in)

    def forward(self, x, x_spa, attention=False):
        # B = x.shape[0]
        ...
        # x = torch.cat((cls_tokens, x_spa.flatten(2).transpose(1, 2)), dim=1)
        # CAFF
        x = torch.cat((x.unsqueeze(1), x_spa.flatten(2).transpose(1, 2)), dim=1)

        # attention

        if attention == True:
            result = self.attention(x, attention=attention)
            out2 = result[0].squeeze(1) * self.gamma
            attn = result[1]
            out = self.attention(x,attention=attention)[0].squeeze(1) * self.gamma
        else:
            # out2 = self.attention(x, attention=attention).squeeze(1) * self.gamma
            out2 = self.attention(x)*self.gamma

        # 1.simple pooling
        # out3 = self.pool(x_spa).squeeze()
...
class glow_coupling_layer(nn.Module):
    ...
    def forward(self, x, attention=False, rev=False):
        # dim,start,length,从dim上取start到start + len长度的Tensor
        # B,C,N -- B,N,C
        x = x[0].transpose(1, 2)
        x_sem = x[:, 0]
        x_spa = x[:, 1:]
        b, spa, channel = x_spa.shape
        h = int(math.sqrt(spa))
        # (b,C,24,24)
        x_spa = x_spa.transpose(1, 2).reshape(b, channel, h, h)
        # x1, x2 =(x[0].narrow(1, 0, self.split_len1),
        #           x[0].narrow(1, self.split_len1, self.split_len2))
        x1, x2 = (x_sem.narrow(1, 0, self.split_len1),
                  x_sem.narrow(1, self.split_len1, self.split_len2))
        x1_spa, x2_spa = (x_spa.narrow(1, 0, self.split_len1),
                          x_spa.narrow(1, self.split_len1, self.split_len2))
        # 一次coupling Layer
        if not rev and attention == False:
            # 将输入的channels输出两倍后，前半部分激活后取指数，后半部分直接t2
            r2 = self.s2(x2, x2_spa, attention=attention)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            # print(s2.shape, x1.shape, t2.shape)
            y1 = self.e(s2) * x1 + t2

            r1 = self.s1(y1, x1_spa, attention=attention)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = self.e(s1) * x2 + t1
        elif not rev and attention == True:
            r2, attn1 = self.s2(x2, x2_spa, attention=attention)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            # print(s2.shape, x1.shape, t2.shape)
            y1 = self.e(s2) * x1 + t2

            r1, attn2 = self.s1(y1, x1_spa, attention=attention)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = self.e(s1) * x2 + t1
        else:  # names of x and y are swapped!
            r1 = self.s1(x1, x1_spa, attention=attention)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = (x2 - t1) / self.e(s1)

            r2 = self.s2(y2, x2_spa, attention=attention)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            y1 = (x1 - t2) / self.e(s2)
        y = torch.cat((y1, y2), 1)
        # 将输出的结果压缩到-1e6到1e6之间
        ...
        out = torch.cat((y.unsqueeze(1), x_spa), dim=1).transpose(1, 2)

        if attention == True:
            atten = torch.cat((attn1, attn2))
            return [out], atten
        else:
            return [out]
    ...
    def jacobian(self, x, rev=False):
        ...
        x1_spa, x2_spa = (x_spa.narrow(1, 0, self.split_len1),
                          x_spa.narrow(1, self.split_len1, self.split_len2))
        if not rev:
            r2 = self.s2(x2, x2_spa)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            y1 = self.e(s2) * x1 + t2

            r1 = self.s1(y1, x1_spa)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]

        else:  # names of x and y are swapped!
            r1 = self.s1(x1, x1_spa)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = (x2 - t1) / self.e(s1)

            r2 = self.s2(y2, x2_spa)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
