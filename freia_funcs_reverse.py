...
class F_attention(nn.Module):
    ...
    def __init__(self, size_in, size, channels_hidden=None, dropout=0.0):
        ...
        self.attention = Attention(dim=size_in, num_heads=4)
...
class glow_coupling_layer(nn.Module):
    def __init__(self, dims_in, F_class=F_attention, F_args={},
                 clamp=3.):
    ...
    def forward(self, x, attention=False, rev=False):
        ...
        x1_sem, x2_sem = (x_sem.narrow(1, 0, self.split_len1),
                          x_sem.narrow(1, self.split_len1, self.split_len2))
        x1, x2 = (x_spa.narrow(1, 0, self.split_len1),
                  x_spa.narrow(1, self.split_len1, self.split_len2))
        # 一次coupling Layer
        if not rev and attention == False:
            # 将输入的channels输出两倍后，前半部分激活后取指数，后半部分直接t2
            r2 = self.s2(x2_sem, x2, attention=attention)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            # print(s2.shape, x1.shape, t2.shape)
            y1 = self.e(s2) * x1.flatten(2) + t2

            r1 = self.s1(x1_sem, y1, attention=attention)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = self.e(s1) * x2.flatten(2) + t1
        elif not rev and attention == True:
            r2, attn1 = self.s2(x2_sem, x2, attention=attention)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            # print(s2.shape, x1.shape, t2.shape)
            y1 = self.e(s2) * x1 + t2

            r1, attn2 = self.s1(x1_sem, y1, attention=attention)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = self.e(s1) * x2 + t1
        else:  # names of x and y are swapped!
            r1 = self.s1(x1_sem, x1, attention=attention)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = (x2 - t1) / self.e(s1)

            r2 = self.s2(x2_sem, y2, attention=attention)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            y1 = (x1 - t2) / self.e(s2)
        y = torch.cat((y1, y2), 1)
        # 将输出的结果压缩到-1e6到1e6之间
        ...
        # B,N,C ---- B,C,N
        out = torch.cat((x_sem.unsqueeze(2), y), dim=2)
        if attention == True:
    ...
    def jacobian(self, x, rev=False):
        ...
        x1, x2 = (x_spa.narrow(1, 0, self.split_len1),
                          x_spa.narrow(1, self.split_len1, self.split_len2))
        if not rev:
            r2 = self.s2(x2_sem, x2)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            y1 = self.e(s2) * x1.flatten(2) + t2

            r1 = self.s1(x1_sem, y1)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]

        else:  # names of x and y are swapped!
            r1 = self.s1(x1_sem, x1)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = (x2 - t1) / self.e(s1)

            r2 = self.s2(x2_sem, y2)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
        # if isinstance(self.s1, F_conv):
        #     jac = (torch.sum(self.log_e(s1), dim=(1, 2, 3))
        #            + torch.sum(self.log_e(s2), dim=(1, 2, 3)))
        if isinstance(self.s1, F_attention):
            jac = (torch.sum(self.log_e(s1), dim=(1, 2))
                   + torch.sum(self.log_e(s2), dim=(1, 2)))
        else:
